...
gt=tianjin_RGG     # random graph type, ER, BA, BA-Tree, RGG
gs=r0.08    # graph specific parameter
...
for (( ind=0; ind < 1; ind++ )) do
	...
	for (( run=0; run < 1; run++ )) do
		...
		python -u main_SIR_mix_random_tianjin_RGG.py --exp_name ${simulate_exp_name} --valid ${valid} --gt ${gt} --n_node ${N} --n_frame ${nf} --batch_size ${bs} --epoch ${ep} --ks ${ks} --kt ${kt} --sconv ${sc} --save ${save} --graph $g --seq ${s1} --start ${skip} --end ${end}
		python -u test_SIR_mix_random_tianjin_RGG.py --load_exp_name ${simulate_exp_name} --exp_name ${real_exp_name} --valid ${valid} --gt ${gt} --n_node ${N} --n_frame ${nf} --batch_size ${bs} --epoch ${ep} --ks ${ks} --kt ${kt} --sconv ${sc} --save ${save} --graph $g --seq ${s2} --start ${skip} --end ${end} | tail -n 1 >> ${output}
