...
# ---------------------------------
# overlay dataset on a RGG graph
# ---------------------------------
def build_network_RGG(df, N, r):
    # Network Definition
...
if __name__ == '__main__':
    ...
    for gid in range(n_graphs):
        g, node_dict, node_data = build_network_RGG(df, N, r)
        ...
        nx.write_edgelist(g, '../data/graph/singapore_RGG_N%s_r%s_g%s.edgelist' % (N, r, gid), data=False)

        with open('../data/SIR/singapore_node_dict_RGG_N%s_r%s_g%s.pickle' % (N, r, gid), 'wb') as f:
            pickle.dump(node_dict, f)

        with open('../data/SIR/SIR_singapore_RGG_N%s_r%s_g%s.pickle' % (N, r, gid), 'wb') as f:
