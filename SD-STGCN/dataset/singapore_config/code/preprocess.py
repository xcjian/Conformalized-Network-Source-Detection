...
if __name__ == '__main__':
    ...
    for gid in range(n_graphs):
        ...
        nx.write_edgelist(g, '../data/graph/singapore_config_N%s_g%s.edgelist' % (N, gid), data=False)

        with open('../data/SIR/singapore_node_dict_config_N%s_g%s.pickle' % (N, gid), 'wb') as f:
            pickle.dump(node_dict, f)

        with open('../data/SIR/SIR_singapore_config_N%s_g%s.pickle' % (N, gid), 'wb') as f:
