...
if __name__ == '__main__':
    ...
    for gid in range(n_graphs):
        g, node_dict, node_data = build_network_ER(df, N, p)
        ...
        nx.write_edgelist(g, '../data/graph/singapore_ER_N%s_p%s_g%s.edgelist' % (N, p, gid), data=False)

        with open('../data/SIR/singapore_node_dict_ER_N%s_p%s_g%s.pickle' % (N, p, gid), 'wb') as f:
            pickle.dump(node_dict, f)

        with open('../data/SIR/SIR_singapore_ER_N%s_p%s_g%s.pickle' % (N, p, gid), 'wb') as f:
