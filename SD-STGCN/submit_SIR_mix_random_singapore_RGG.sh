...
gt=singapore_RGG     # random graph type, ER, BA, BA-Tree, RGG
...
for (( ind=0; ind < 5; ind++ )) do
	...
	for (( run=0; run < 5; run++ )) do
		...
		python -u main_SIR_mix_random_singapore_RGG.py --exp_name ${simulate_exp_name} --valid ${valid} --gt ${gt} --n_node ${N} --n_frame ${nf} --batch_size ${bs} --epoch ${ep} --ks ${ks} --kt ${kt} --sconv ${sc} --save ${save} --graph $g --seq ${s1} --start ${skip} --end ${end}
		python -u test_SIR_mix_random_singapore_RGG.py --load_exp_name ${simulate_exp_name} --exp_name ${real_exp_name} --valid ${valid} --gt ${gt} --n_node ${N} --n_frame ${nf} --batch_size ${bs} --epoch ${ep} --ks ${ks} --kt ${kt} --sconv ${sc} --save ${save} --graph $g --seq ${s2} --start ${skip} --end ${end} | tail -n 1 >> ${output}
