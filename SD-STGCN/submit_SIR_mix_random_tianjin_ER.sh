...
nsrc1=1-30 # train simulation nsrc
...
for (( ind=0; ind < 1; ind++ )) do
	...
	s1="./dataset/${gt}/data/SIR/SIR_Rzero${Rzero1}_nsrc${nsrc1}_gamma${gamma1}_ls${ns}_nf${nf}_N${N}_${gs}_g${ind}_entire.pickle"
	...
	for (( run=0; run < 1; run++ )) do
		...
		python -u main_SIR_mix_random_tianjin_ER.py --exp_name ${simulate_exp_name} --valid ${valid} --gt ${gt} --n_node ${N} --n_frame ${nf} --batch_size ${bs} --epoch ${ep} --ks ${ks} --kt ${kt} --sconv ${sc} --save ${save} --graph $g --seq ${s1} --start ${skip} --end ${end}
		python -u test_SIR_mix_random_tianjin_ER.py --load_exp_name ${simulate_exp_name} --exp_name ${real_exp_name} --valid ${valid} --gt ${gt} --n_node ${N} --n_frame ${nf} --batch_size ${bs} --epoch ${ep} --ks ${ks} --kt ${kt} --sconv ${sc} --save ${save} --graph $g --seq ${s2} --start ${skip} --end ${end} | tail -n 1 >> ${output}
